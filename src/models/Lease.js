...
const Lease = sequelize.define('Lease', {
  ...
  tenant1Id: { type: DataTypes.INTEGER, allowNull: true, references: { model: 'tenants', key: 'id' } },
  tenant2Id: { type: DataTypes.INTEGER, allowNull: true, references: { model: 'tenants', key: 'id' } },
  tenant3Id: { type: DataTypes.INTEGER, allowNull: true, references: { model: 'tenants', key: 'id' } },
  tenant4Id: { type: DataTypes.INTEGER, allowNull: true, references: { model: 'tenants', key: 'id' } },
  startDate: { type: DataTypes.DATE, allowNull: false },
  endDate: { type: DataTypes.DATE, allowNull: false },
  monthlyRent: { type: DataTypes.DECIMAL(10, 2), allowNull: false, validate: { min: 0.01 } },
  totalLeaseValue: { type: DataTypes.DECIMAL(10, 2), allowNull: true, comment: 'Total value of the entire lease term' },
  totalAmountPaid: { type: DataTypes.DECIMAL(10, 2), defaultValue: 0.00, comment: 'Total amount paid by tenants to date' },
  totalAmountRemaining: { type: DataTypes.DECIMAL(10, 2), allowNull: true, comment: 'Remaining amount to be paid on lease' },
  securityDeposit: { type: DataTypes.DECIMAL(10, 2), defaultValue: 0.00 },
  petDeposit: { type: DataTypes.DECIMAL(10, 2), defaultValue: 0.00 },
  lateFeeAmount: { type: DataTypes.DECIMAL(10, 2), defaultValue: 0.00 },
  lateFeeDaysAfterDue: { type: DataTypes.INTEGER, defaultValue: 5 },
  rentDueDay: { type: DataTypes.INTEGER, defaultValue: 1, validate: { min: 1, max: 31 } },
  status: { type: DataTypes.ENUM('active','expired','terminated','pending'), defaultValue: 'pending' },
  leaseType: { type: DataTypes.ENUM('fixed-term','month-to-month'), defaultValue: 'fixed-term' },
  notes: { type: DataTypes.TEXT, allowNull: true },
  signedDate: { type: DataTypes.DATE, allowNull: true }
}, {
  tableName: 'leases',
  timestamps: true,
  indexes: [
